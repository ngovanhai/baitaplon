
  <footer class="footer-page">
    <div class="footer-top">
      <div class="container">
        <div class="row">
          <div class="col-md-8 text-left">
            <h3>
              <span id="dnn_dnnCopyright_lblCopyright" class="SkinObject"
                >© 2020 - TRƯỜNG ĐẠI HỌC THỦY LỢI</span
              >
            </h3>
          </div>
          <div class="col-md-4 text-right">
            <a
              href="https://www.facebook.com/daihocthuyloi1959/?fref=ts"
              target="_blank"
              ><img src="./../../media/image/fb-icon.png"
              
            /></a>
            <a href="https://www.youtube.com/user/daihocthuyloi" target="_blank"
              ><img src="./../../media/image/ytb-icon.png"
            /></a>
            <a href="https://twitter.com/Daihocthuyloihn" target="_blank"
              ><img src="./../../media/image/twitter-icon.png"
            /></a>
          </div>
        </div>
      </div>
    </div>
    <!-- /.footer-top -->
    <div class="footer-middle">
      <div class="container">
        <div id="dnn_footerPane">
          <div class="DnnModule DnnModule-DNN_HTML DnnModule-1532">
            <a name="1532"></a>
            <div id="dnn_ctr1532_ContentPane">
              <!-- Start_Module_1532 -->
              <div
                id="dnn_ctr1532_ModuleContent"
                class="DNNModuleContent ModDNNHTMLC"
              >
                <div id="dnn_ctr1532_HtmlModule_lblContent" class="Normal">
                  <div class="row">
                    <div class="col-md-4">
                      <a href="http://ts.tlu.edu.vn/#" target="_blank"
                        ><img
                          alt="Image"
                          src="http://localhost/baitaplon/media/image/TLU-map.png"
                        />
                      </a>
                    </div>

                    <div class="col-md-8">
                      <div class="row">
                        <div class="col-md-6">
                          <div class="address">
                            TRƯỜNG ĐẠI HỌC THỦY LỢI<br />
                            Địa chỉ : 175 TÂY SƠN, ĐỐNG ĐA, HÀ NỘI.<br />
                            Điện thoại: (024) 3852 2201 - Fax: (024) 3563
                            3351<br />
                            Email:
                            <a href="http://ts.tlu.edu.vn/#"
                              >phonghcth@tlu.edu.vn</a
                            >
                          </div>

                          <hr />
                          <div class="address">
                            PHÂN HIỆU ĐẠI HỌC THỦY LỢI<br />
                            Địa chỉ : Số 2 - Đường Trường Sa - P.17 - Q.Bình
                            Thạnh - Tp.Hồ Chí Minh<br />
                            Điện thoại: (84)028.38&nbsp;400 532 - Fax:
                            (84)028.38 400542<br />
                            Email:
                            <a href="http://ts.tlu.edu.vn/#">cs2@tlu.edu.vn</a>
                          </div>
                        </div>

                        <div class="col-md-6">
                          <div class="address">
                            PHÂN HIỆU ĐẠI HỌC THỦY LỢI<br />
                            Phường An Thạnh - TX Thuận An - Tỉnh Bình Dương<br />
                            Điện thoại: (84).650.3748 620<br />
                            Fax:(84).650.3833 489
                          </div>

                          <hr />
                          <div class="address">
                            VIỆN ĐÀO TẠO VÀ KHOA HỌC ỨNG DỤNG MIỀN TRUNG<br />
                            Địa chỉ: Số 115 Trần Phú, Thành phố&nbsp;Phan Rang,
                            Tỉnh Ninh Thuận<br />
                            Điện thoại: 02593.823 027, 02593.823 028<br />
                            Email:
                            <a href="http://ts.tlu.edu.vn/#">
                              trungtamdh2@tlu.edu.vn</a
                            >
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <!-- End_Module_1532 -->
            </div>
          </div>
        </div>
      </div>
    </div>
    <!-- /.footer-middle -->
  </footer>

